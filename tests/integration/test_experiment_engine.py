...
def _write_atomic_fixture(tmp_path: Path) -> Path:
    ...
    header = "event,oEffect,oReact,oWant,xAttr,xEffect,xIntent,xNeed,xReact,xWant\n"
    train_rows = [
        f"train event {idx},['effect {idx}'],[],[],[],[],[],[],[],['want {idx}']\n"
        for idx in range(5)
    ]
    dev_rows = [
        f"dev event {idx},['dev effect {idx}'],[],[],[],[],[],[],[],['dev want {idx}']\n"
        for idx in range(2)
    ]
    test_rows = [
        f"heldout event {idx},['heldout effect {idx}'],[],[],[],[],[],[],[],['heldout want {idx}']\n"
        for idx in range(3)
    ]
    (data_dir / "v4_atomic_trn.csv").write_text(header + "".join(train_rows), encoding="utf-8")
    (data_dir / "v4_atomic_dev.csv").write_text(header + "".join(dev_rows), encoding="utf-8")
    (data_dir / "v4_atomic_tst.csv").write_text(header + "".join(test_rows), encoding="utf-8")
    return data_dir
...
def _cfg(tmp_path: Path):
    ...
    cfg.experiment.relations.selected = ["oEffect", "xWant"]
    return cfg
...
def test_relation_filtering_and_evaluation_limit_are_independent(tmp_path: Path) -> None:
    ...
    cfg.experiment.evaluation.depthrank.heads_per_relation = 1

    out = ExperimentRunner(cfg).run()
    mi = (out / "mi_scores.csv").read_text()

    assert "xWant" in mi
    assert "heldout event 1" not in mi
    assert ",1," in mi
